...
def get_task_metadata():
    return {
        ...
        "description": "MLP classifier on Wine with SGD and cosine-annealing schedule",
        "input_dim": 13,
        "output_dim": 3,
    }
...
def train(model, train_loader, val_loader, device, **kwargs):
    ...
    for epoch in range(epochs):
        for xb, yb in train_loader:
            ...
            optimizer.zero_grad()
            loss = nn.functional.cross_entropy(model(xb), yb, label_smoothing=0.05)
            loss.backward()
            optimizer.step()
        scheduler.step()
        if (epoch + 1) % 50 == 0:
            ...
            print(
                f"Epoch {epoch+1}/{epochs}  Val Acc: {metrics['accuracy']:.4f}  "
...
if __name__ == "__main__":
    set_seed(42)
    device = get_device()
    print("Task: MLP (Wine + SGD + CosineAnnealing)")
    print(f"Device: {device}")

    train_loader, val_loader = make_dataloaders()
    model = build_model(device=device)
    train(model, train_loader, val_loader, device, epochs=150, lr=0.03)
